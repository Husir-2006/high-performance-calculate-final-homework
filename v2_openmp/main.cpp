...
// 构建场景：与V1完全一致，保证渲染结果可比对（同样的随机种子序列除外，
// 随机部分球体分布在并行/串行下不要求逐像素位级一致，只要求统计意义上一致）
...
int main(int argc, char** argv) {
    ...
    // 像素结果先写入framebuffer，再统一按顺序输出PPM，
    // 这样并行写入互不干扰，避免多线程竞争同一个文件流
    ...
    // 行级并行：每行像素独立，互不依赖，适合按行动态调度
    // schedule(dynamic, 1) 避免负载不均：边缘/天空区域光线追踪深度浅，计算更快
    ...
    for (int j = 0; j < HEIGHT; j++) {
        for (int i = 0; i < WIDTH; i++) {
            ...
            // 行 j 对应PPM中从上到下第 (HEIGHT-1-j) 行（PPM按从上到下写，
            // 渲染习惯按从下到上的图像行j），保持与V1输出方向一致
    ...
    for (int idx = 0; idx < WIDTH * HEIGHT; idx++) {
        write_pixel(out, framebuffer[idx], SAMPLES);
    }
    out.close();

    std::cerr << "Threads=" << num_threads << " Render time: " << elapsed << " s\n";
    std::cout << "Threads=" << num_threads << " Render time: " << elapsed << " s" << std::endl;
